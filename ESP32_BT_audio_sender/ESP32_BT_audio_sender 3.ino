...
void setup() {
  ...
  while (!isLastFile(songFile = nextFile(&songFolder)))
  {
...
void mp3ReadIR()
{
  if (IR_cbData.justWritten && !IR_cbData.isRepeat)
  {
    if (IR_cbData.Command == SHUFFLE_CMD)
    {
      if (!shuffle)
        shuffle = true;
      else
    ...
    else if (IR_cbData.Command == NEXT_TRACK_CMD)
    {
      if (!next)
        next = true;
    }

    else if (IR_cbData.Command == PREV_TRACK_CMD)
    {

      if (!prev)
        prev = true;
    }
    else if (IR_cbData.Command == VOLUME_DOWN_CMD)
    {
      Volume -= 5;
      if (Volume <= 15)
        Volume = 15;
    ...
    else if (IR_cbData.Command == VOLUME_UP_CMD)
    {
      Volume += 5;
      if (Volume >= 100)
        Volume = 100;
    ...
    else if (IR_cbData.Command == PLAY_PAUSE_CMD)
    {
      if (paused)
      {
