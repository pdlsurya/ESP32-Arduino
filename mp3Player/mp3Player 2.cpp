...
bool mp3Player_init(const char *songsPath)
{
    ...
    */
    ...
    while (isValidFile(songFile = nextFile(&songFolder)))
    {
        if (songFile.DIR_Name[0] != '_' && songFile.DIR_Name[1] != '.' && !isDirectory(songFile) && isMp3File(fileName))
...
void mp3PlayerProcess()
{
    ...
    if (!paused)
    {
        if (!play_in_progress)
        {
            ...
            if (songFile.DIR_Name[0] != '_' && songFile.DIR_Name[1] != '.' && !isDirectory(songFile))
            {
